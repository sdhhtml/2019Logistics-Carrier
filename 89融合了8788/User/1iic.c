...
{
	IIC_Start1();	
	IIC_SendByte1(I2C_Addr);	 //发送从机地址
	if(IIC_WaitAck1()) //如果从机未应答则数据发送失败
	{
		IIC_Stop1();
		return 1;
	}
	IIC_SendByte1(reg); //发送寄存器地址
	IIC_WaitAck1();	  
	
	IIC_Start1();
	IIC_SendByte1(I2C_Addr+1); //进入接收模式			   
	IIC_WaitAck1();
	*buf=IIC_ReadByte1(0);	   
	IIC_Stop1(); //产生一个停止条件
	return 0;
...
uint8_t IIC_ReadMultByteFromSlave1(uint8_t dev, uint8_t reg, uint8_t length, uint8_t *data)
{
	...
	IIC_WaitAck1();	  
	IIC_Start1();
	IIC_SendByte1(dev+1); //进入接收模式	
	IIC_WaitAck1();
	for(count=0;count<length;count++)
	{
		if(count!=(length-1))
			temp = IIC_ReadByte1(1); //带ACK的读数据
		else  
			temp = IIC_ReadByte1(0); //最后一个字节NACK
